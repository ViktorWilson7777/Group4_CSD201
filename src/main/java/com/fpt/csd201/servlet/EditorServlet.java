...
public class EditorServlet extends HttpServlet {
        ...
        try {
            switch (action) {
                ...
                case "redo":
                    ...
                    break;
                case "save":
                    handleSave(json, editor, manager, session, result);
                    break;
                case "load":
                    handleLoad(json, editor, manager, session, result);
                    break;
                case "search":
    ...
    private void handleSave(JsonObject json, TextEditor editor,
                             UndoRedoManager manager, HttpSession session,
                             Map<String, Object> result) {
        // Accept content from frontend to ensure sync (especially after server restart)
        if (json.has("content") && !json.get("content").isJsonNull()) {
            String contentFromClient = json.get("content").getAsString();
            // Only update editor if content differs (avoids unnecessary state change)
            if (!contentFromClient.equals(editor.getContent())) {
                editor.setContent(contentFromClient);
                // Reset history since editor content was externally set
    ...
    private void handleLoad(JsonObject json, TextEditor editor,
                             UndoRedoManager manager, HttpSession session,
                             Map<String, Object> result) {
        // Try session first (fast path, same session)
        String saved = (String) session.getAttribute(SESSION_SAVED);

        // If session is empty, try content from frontend (localStorage backup)
        if (saved == null && json.has("content") && !json.get("content").isJsonNull()) {
            String clientContent = json.get("content").getAsString();
        ...
        manager.setHistoryModel(createFreshModel(manager.getHistoryModel().getModelName()));
        // Persist to session so subsequent loads work without localStorage
        session.setAttribute(SESSION_SAVED, saved);
        session.removeAttribute(SESSION_LAST_GROUP_ID);
    ...
    private HistoryStrategy createFreshModel(String name) {
        switch (name) {
            ...
            case "BoundedStackHistory":
                return new BoundedStackHistory();
            case "LRUStackHistory":
                return new LRUStackHistory();
